...
pub fn import_xlsx_core(path: String) -> Result<ImportWorkbookResult, String> {
    ...
    warnings.push(CompatibilityWarning {
        ...
        message:
            "xlsx PoC import: borders, number formats, merges, and named ranges are not yet preserved"
                .to_string(),
        affected_sheets: None,
    });
...
pub fn workbook_export_xlsx(
    ...
    warnings.push(CompatibilityWarning {
        ...
        message: format!(
            "xlsx PoC export: {sheet_count} sheets, {cell_count} cells, {formula_count} formulas. Borders, merges, column widths, named ranges, and rich text are not yet preserved."
        ),
        affected_sheets: None,
    });
